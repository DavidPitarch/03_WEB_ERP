...
import type { CreateParteRequest, UploadCompleteRequest } from '@erp/types';
...
async function getOperarioId(supabase: any, userId: string): Promise<string | null> {
...
async function getAssignedExpediente(supabase: any, expedienteId: string, operarioId: string) {
  const { data } = await supabase
    .from('expedientes')
...
operatorRoutes.get('/me/agenda', async (c) => {
  ...
  const parteCitaIds = new Set((partes ?? []).map((p: any) => p.cita_id));

  const agenda = (citas ?? []).map((c: any) => ({
    cita_id: c.id,
...
operatorRoutes.get('/claims/:id/timeline', async (c) => {
  ...
  const timeline = [
    ...(commsRes.data ?? []).map((c: any) => ({ ...c, timeline_type: 'comunicacion' })),
    ...(citasRes.data ?? []).map((c: any) => ({ ...c, timeline_type: 'cita' })),
...
operatorRoutes.post('/claims/:id/parts', async (c) => {
  ...
  const body = await c.req.json<CreateParteRequest>();
  ...
  const errors: string[] = [];
  if (!body.trabajos_realizados?.trim()) errors.push('trabajos_realizados requerido');
  if (!body.resultado) errors.push('resultado requerido');
  if (body.resultado === 'ausente' && !body.motivo_resultado) errors.push('motivo requerido cuando resultado = ausente');
  if (body.resultado === 'requiere_material' && !body.motivo_resultado) errors.push('motivo requerido cuando resultado = requiere_material');
