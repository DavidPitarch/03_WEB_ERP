...
export const pedidosRoutes = new Hono<{ Bindings: Env }>();
...
async function insertHistorialPedido(
  supabase: any,
  params: {
    pedido_id: string;
...
pedidosRoutes.get('/caducados', async (c) => {
  ...
  if (dias_min) query = query.gte('dias_caducado', Number(dias_min));
...
pedidosRoutes.get('/:id', async (c) => {
  ...
  if (error || !data) return c.json(err('NOT_FOUND', 'Pedido no encontrado'), 404);
  return c.json({ data, error: null });
});
...
pedidosRoutes.post('/', async (c) => {
  const supabase = c.get('supabase');
  const user = c.get('user');
  const body = await c.req.json<import('@erp/types').CreatePedidoRequest>();
  ...
  if (pedidoErr) return c.json(err('DB_ERROR', pedidoErr.message), 500);
  ...
  if (lineasErr) return c.json(err('DB_ERROR', lineasErr.message), 500);
...
pedidosRoutes.post('/:id/enviar', async (c) => {
  ...
  if (!pedido) return c.json(err('NOT_FOUND', 'Pedido no encontrado'), 404);
  ...
  let email_result: { success: boolean; dryRun: boolean; error?: string } | null = null;
  const proveedor = (pedidoDetail as any)?.proveedores;
  const expediente = (pedidoDetail as any)?.expedientes;
  const lineas = ((pedidoDetail as any)?.lineas ?? []) as Array<{ descripcion: string; cantidad: number; unidad: string }>;

  if (proveedor?.email) {
    email_result = await sendPedidoEmail(
      supabase,
      c.env.RESEND_API_KEY,
      {
        id,
        numero_pedido: (pedidoDetail as any)?.numero_pedido ?? pedido.numero_pedido,
...
pedidosRoutes.post('/:id/confirmar', async (c) => {
  ...
  if (!token) return c.json(err('VALIDATION', 'token es requerido'), 422);
