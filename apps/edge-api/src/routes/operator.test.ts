...
function validateOperarioAccess(userId: string, operarioUserId: string): boolean {
...
function isExpedienteAssigned(expedienteOperarioId: string, requestOperarioId: string): boolean {
...
describe('Timeline update after parte', () => {
  it('creates comunicacion entry for parte received', () => {
    const resultado: string = 'completada';
...
describe('Cita estado update after parte', () => {
  ...
  it('marks cita as no_show for ausente', () => {
    const resultado: string = 'ausente';
    const newEstado = resultado === 'ausente' ? 'no_show' : 'realizada';
