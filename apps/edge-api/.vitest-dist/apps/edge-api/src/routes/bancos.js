...
export const bancosRoutes = new Hono();
// ─── Helper ─────────────────────────────────────────────────────────────────
...
function calcFechaVencimiento(emisionDate, diasPago, diaFijoMes) {
...
function calcularSugerencias(movimientos, referencias, toleranciaEur = 0) {
    const sugerencias = [];
    for (const mov of movimientos) {
        ...
        for (const ref of referencias) {
            ...
            if (mov.concepto_banco.toUpperCase().includes(ref.referencia.toUpperCase())) {
                score = 100;
...
function parseCSVExtracto(text) {
    const errors = [];
    const lines = text
        .split(/\r?\n/)
        .map((l) => l.trim())
...
bancosRoutes.get('/cuentas/:id', async (c) => {
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error || !data)
        return c.json(err('NOT_FOUND', 'Cuenta bancaria no encontrada'), 404);
    return c.json({ data, error: null });
});
...
bancosRoutes.put('/cuentas/:id', async (c) => {
    ...
    if (body.entidad !== undefined)
        update.entidad = body.entidad;
    if (body.bic_swift !== undefined)
        update.bic_swift = body.bic_swift;
...
bancosRoutes.post('/cuentas/:id/principal', async (c) => {
    ...
    if (!cuenta)
        return c.json(err('NOT_FOUND', 'Cuenta bancaria no encontrada'), 404);
    if (!cuenta.activa)
        return c.json(err('CUENTA_INACTIVA', 'No se puede marcar como principal una cuenta inactiva'), 422);
    await supabase
        .from('cuentas_bancarias')
...
bancosRoutes.put('/perfiles/:id', async (c) => {
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error || !data)
        return c.json(err('NOT_FOUND', 'Perfil de cobro no encontrado'), 404);
    await insertAudit(supabase, {
        tabla: 'perfiles_cobro',
        registro_id: id,
        accion: 'UPDATE',
...
bancosRoutes.get('/referencias/:id', async (c) => {
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error || !data)
        return c.json(err('NOT_FOUND', 'Referencia de cobro no encontrada'), 404);
    return c.json({ data, error: null });
});
...
bancosRoutes.post('/referencias/generar', async (c) => {
    ...
    const { data: factura } = await supabase
        ...
        .single();
    if (!factura)
        return c.json(err('NOT_FOUND', 'Factura no encontrada'), 404);
    if (factura.estado === 'anulada')
        return c.json(err('FACTURA_ANULADA', 'No se puede generar referencia para una factura anulada'), 422);
    if (factura.referencia_cobro_id)
        return c.json(err('REF_EXISTENTE', 'La factura ya tiene una referencia de cobro asignada'), 409);
    ...
    const empresaNombre = factura.empresas_facturadoras?.nombre ?? '';
    ...
    const { data: refCobro, error: refErr } = await supabase
        ...
        .single();
    if (refErr)
        return c.json(err('DB_ERROR', refErr.message), 500);
...
bancosRoutes.post('/referencias/:id/anular', async (c) => {
    ...
    const { data: ref } = await supabase
        ...
        .single();
    if (!ref)
        return c.json(err('NOT_FOUND', 'Referencia no encontrada'), 404);
    if (ref.estado === 'cobrada')
        return c.json(err('REF_COBRADA', 'No se puede anular una referencia cobrada'), 422);
    if (ref.estado === 'anulada')
        return c.json(err('REF_ANULADA', 'La referencia ya está anulada'), 409);
    const { data, error } = await supabase
        .from('referencias_cobro')
...
bancosRoutes.get('/movimientos', async (c) => {
    ...
    if (estado_concil)
        query = query.eq('estado_concil', estado_concil);
    if (desde)
...
bancosRoutes.post('/movimientos', async (c) => {
    ...
    if (error) {
        if (error.code === '23505')
            return c.json(err('DUPLICADO', 'Movimiento duplicado (misma cuenta, fecha, importe y referencia)'), 409);
        return c.json(err('DB_ERROR', error.message), 500);
    }
    await insertAudit(supabase, {
...
bancosRoutes.post('/movimientos/importar', async (c) => {
    ...
    if (!cuentaId)
        return c.json(err('VALIDATION', 'cuenta_id es requerido'), 422);
    if (!fichero)
        return c.json(err('VALIDATION', 'fichero es requerido'), 422);
    ...
    try {
        ...
        const tolerancia = (reglas ?? []).reduce((max, r) => Math.max(max, r.tolerancia_eur ?? 0), 0);
        ...
        const autoRule = (reglas ?? []).find((r) => r.accion === 'aplicar_automatico');
        if (autoRule) {
...
async function aplicarConciliacion(supabase, s, actor_id, tipo) {
    ...
    if (ref?.factura_id) {
        ...
        const totalCobrado = (pagos ?? []).reduce((sum, p) => sum + (p.importe ?? 0), 0);
        const { data: factura } = await supabase
            .from('facturas')
...
bancosRoutes.get('/conciliacion/sugerencias', async (c) => {
    ...
    let movQuery = supabase
        ...
        .eq('estado_concil', 'pendiente');
    if (cuenta_id)
        movQuery = movQuery.eq('cuenta_id', cuenta_id);
    const { data: movimientos } = await movQuery;
    if (!movimientos || movimientos.length === 0) {
        return c.json({ data: [], error: null });
    }
    // Determinar empresa_id desde la cuenta si no se pasó
    const empId = empresa_id ?? movimientos[0].cuentas_bancarias?.empresa_id;
...
bancosRoutes.post('/conciliacion/aplicar', async (c) => {
    ...
    if (!mov)
        return c.json(err('NOT_FOUND', 'Movimiento bancario no encontrado'), 404);
    if (!ref)
        return c.json(err('NOT_FOUND', 'Referencia de cobro no encontrada'), 404);
    if (mov.estado_concil === 'conciliado')
        return c.json(err('YA_CONCILIADO', 'El movimiento ya está conciliado'), 409);
    if (ref.estado === 'cobrada')
        return c.json(err('REF_COBRADA', 'La referencia ya está cobrada'), 409);
    if (ref.estado === 'anulada')
        return c.json(err('REF_ANULADA', 'La referencia está anulada'), 422);
    const result = await aplicarConciliacion(supabase, {
        movimiento_id: body.movimiento_id,
        referencia_cobro_id: body.referencia_cobro_id,
        importe_movimiento: mov.importe,
        importe_referencia: ref.importe,
    }, user.id, body.tipo ?? 'manual');
    if (!result.ok)
        return c.json(err('DB_ERROR', result.error ?? 'Error al conciliar'), 500);
...
bancosRoutes.post('/conciliacion/ignorar', async (c) => {
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error || !data)
        return c.json(err('NOT_FOUND', 'Movimiento no encontrado o ya conciliado'), 404);
    await insertAudit(supabase, {
        tabla: 'movimientos_bancarios',
...
bancosRoutes.get('/facturas/:factura_id/cobro', async (c) => {
    ...
    if (factura.referencia_cobro_id) {
        const { data } = await supabase
            .from('referencias_cobro')
            .select('*')
            .eq('id', factura.referencia_cobro_id)
            .single();
        referencia = data;
    }
    // Pagos registrados
    ...
    return c.json({
        data: {
            factura: {
                ...
                cobrada_at: factura.cobrada_at,
                fecha_vencimiento: factura.fecha_vencimiento,
            },
            perfil_cobro: factura.perfiles_cobro ?? null,
            cuenta_cobro: factura.cuentas_bancarias ?? null,
            referencia,
            pagos: pagos ?? [],
            apuntes: apuntes ?? [],
            total_cobrado: totalCobrado,
            pendiente: Math.max(0, (factura.total ?? 0) - totalCobrado),
        },
        error: null,
    });
});
