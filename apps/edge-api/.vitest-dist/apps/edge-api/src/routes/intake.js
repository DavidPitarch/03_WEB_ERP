...
intakeRoutes.post('/claims', async (c) => {
    ...
    if (!rootCheck.ok)
        return validationError(c, rootCheck.errors);
    // Validación del sub-objeto asegurado
    const aseguradoCheck = validate(body.asegurado ?? {}, {
    ...
    if (!compania) {
        return c.json({
            data: { status: 'validation_error', errors: [`Compania con codigo '${body.compania_codigo}' no encontrada`] },
            error: null,
        }, 422);
    }
    ...
    const { data: existingAsegurado } = await supabase
        ...
        .maybeSingle();
    try {
        const exp = await createExpedienteCommand(supabase, {
