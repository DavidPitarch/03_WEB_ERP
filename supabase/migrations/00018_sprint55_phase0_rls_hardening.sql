...
RETURNS boolean AS $$
  SELECT COALESCE(public.user_roles() && required_roles, false);
$$ LANGUAGE sql SECURITY DEFINER STABLE;

CREATE OR REPLACE FUNCTION public.current_operario_id()
RETURNS uuid AS $$
  SELECT o.id
  FROM operarios o
...
CREATE OR REPLACE FUNCTION public.current_perito_id()
RETURNS uuid AS $$
  SELECT p.id
  FROM peritos p
...
CREATE OR REPLACE FUNCTION public.can_access_vp(vp_id uuid)
RETURNS boolean AS $$
  SELECT
    public.has_any_role(ARRAY['admin', 'supervisor', 'tramitador', 'financiero', 'direccion'])
    OR EXISTS (
      SELECT 1
      FROM vp_videoperitaciones vp
      WHERE vp.id = vp_id
        AND vp.perito_id = public.current_perito_id()
    );
$$ LANGUAGE sql SECURITY DEFINER STABLE;

CREATE OR REPLACE FUNCTION public.can_access_vp_artifact(artefacto_id uuid)
RETURNS boolean AS $$
  SELECT
    public.has_any_role(ARRAY['admin', 'supervisor', 'tramitador', 'financiero', 'direccion'])
    OR EXISTS (
...
DROP POLICY IF EXISTS series_facturacion_staff_select ON series_facturacion;
DROP POLICY IF EXISTS series_facturacion_finance_manage ON series_facturacion;
...
DROP POLICY IF EXISTS seguimiento_cobro_staff_select ON seguimiento_cobro;
DROP POLICY IF EXISTS seguimiento_cobro_finance_manage ON seguimiento_cobro;
DROP POLICY IF EXISTS staff_read_facturas ON facturas;
DROP POLICY IF EXISTS admin_financiero_manage_facturas ON facturas;
DROP POLICY IF EXISTS facturas_staff_select ON facturas;
DROP POLICY IF EXISTS facturas_finance_manage ON facturas;
DROP POLICY IF EXISTS staff_read_pagos ON pagos;
DROP POLICY IF EXISTS lineas_factura_staff_select ON lineas_factura;
DROP POLICY IF EXISTS lineas_factura_finance_manage ON lineas_factura;
DROP POLICY IF EXISTS pagos_staff_select ON pagos;
DROP POLICY IF EXISTS pagos_finance_manage ON pagos;
DROP POLICY IF EXISTS auditoria_staff_select ON auditoria;
DROP POLICY IF EXISTS eventos_dominio_staff_select ON eventos_dominio;
...
DROP POLICY IF EXISTS vp_webhook_logs_admin_all ON vp_webhook_logs;
DROP POLICY IF EXISTS vp_artefactos_office_all ON vp_artefactos;
DROP POLICY IF EXISTS vp_artefactos_perito_select ON vp_artefactos;
...
DROP POLICY IF EXISTS vp_accesos_artefacto_staff_select ON vp_accesos_artefacto;
DROP POLICY IF EXISTS vp_accesos_artefacto_allowed_insert ON vp_accesos_artefacto;
DROP POLICY IF EXISTS vp_transcripciones_office_all ON vp_transcripciones;
DROP POLICY IF EXISTS vp_transcripciones_perito_select ON vp_transcripciones;
...
DROP POLICY IF EXISTS vp_informes_office_select ON vp_informes;
DROP POLICY IF EXISTS vp_informes_edit_own_scope ON vp_informes;
DROP POLICY IF EXISTS vp_informes_update_own_scope ON vp_informes;
...
DROP POLICY IF EXISTS vp_informe_versiones_select_scoped ON vp_informe_versiones;
DROP POLICY IF EXISTS vp_informe_versiones_insert_scoped ON vp_informe_versiones;
...
DROP POLICY IF EXISTS vp_valoraciones_office_select ON vp_valoraciones;
DROP POLICY IF EXISTS vp_valoraciones_office_manage ON vp_valoraciones;
...
DROP POLICY IF EXISTS vp_valoracion_lineas_select_scoped ON vp_valoracion_lineas;
DROP POLICY IF EXISTS vp_valoracion_lineas_manage_scoped ON vp_valoracion_lineas;
...
CREATE POLICY series_facturacion_finance_manage ON series_facturacion FOR ALL
  USING (public.has_any_role(ARRAY['admin', 'financiero']))
  ...
  USING (
    EXISTS (
      ...
      WHERE f.id = lineas_factura.factura_id
        AND public.has_any_role(ARRAY['admin', 'supervisor', 'tramitador', 'financiero', 'direccion'])
    )
  );
...
CREATE POLICY lineas_factura_finance_manage ON lineas_factura FOR ALL
  USING (
    EXISTS (
      ...
      FROM facturas f
      WHERE f.id = lineas_factura.factura_id
        AND public.has_any_role(ARRAY['admin', 'financiero'])
    )
  )
  WITH CHECK (
    EXISTS (
...
CREATE POLICY auditoria_staff_select ON auditoria FOR SELECT
  USING (public.has_any_role(ARRAY['admin', 'supervisor', 'direccion']));
...
CREATE POLICY vp_webhook_logs_admin_all ON vp_webhook_logs FOR ALL
  USING (public.has_any_role(ARRAY['admin', 'supervisor']))
  WITH CHECK (public.has_any_role(ARRAY['admin', 'supervisor']));
...
CREATE POLICY vp_artefactos_perito_select ON vp_artefactos FOR SELECT
  USING (public.can_access_vp_artifact(id));
  ...
  USING (
    EXISTS (
      ...
      WHERE art.id = vp_transcripciones.artefacto_id
        AND public.can_access_vp_artifact(art.id)
    )
  );
...
CREATE POLICY vp_accesos_artefacto_allowed_insert ON vp_accesos_artefacto FOR INSERT
  WITH CHECK (
    ...
    AND (
      public.has_any_role(ARRAY['admin', 'supervisor', 'tramitador', 'financiero', 'direccion'])
      OR public.can_access_vp_artifact(artefacto_id)
    )
  );

CREATE POLICY vp_informes_office_select ON vp_informes FOR SELECT
  USING (public.can_access_vp(videoperitacion_id));

CREATE POLICY vp_informes_edit_own_scope ON vp_informes FOR INSERT
  WITH CHECK (public.can_access_vp(videoperitacion_id));
  ...
  USING (
    EXISTS (
      ...
      WHERE i.id = vp_informe_versiones.informe_id
        AND public.can_access_vp(i.videoperitacion_id)
    )
  );
  ...
  USING (
    EXISTS (
      ...
      WHERE v.id = vp_valoracion_lineas.valoracion_id
        AND public.can_access_vp(v.videoperitacion_id)
    )
  );
...
CREATE POLICY vp_documento_final_select_scoped ON vp_documento_final FOR SELECT
  USING (
    public.can_access_vp(videoperitacion_id)
    OR public.has_any_role(ARRAY['admin', 'supervisor', 'financiero', 'direccion'])
  );

CREATE POLICY vp_documento_final_manage_scoped ON vp_documento_final FOR INSERT
  WITH CHECK (
    public.has_any_role(ARRAY['admin', 'supervisor', 'perito'])
...
CREATE POLICY vp_facturas_finance_insert ON vp_facturas FOR INSERT
  WITH CHECK (public.has_any_role(ARRAY['admin', 'supervisor', 'financiero']));
...
CREATE POLICY vp_envios_finance_update ON vp_envios FOR UPDATE
  USING (public.has_any_role(ARRAY['admin', 'supervisor', 'financiero']))
  WITH CHECK (public.has_any_role(ARRAY['admin', 'supervisor', 'financiero']));
