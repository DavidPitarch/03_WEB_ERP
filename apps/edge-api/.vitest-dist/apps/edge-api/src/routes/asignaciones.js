...
export const asignacionesRoutes = new Hono();
...
asignacionesRoutes.get('/cola', async (c) => {
    ...
    let query = supabase
        .from('expedientes')
        .select('*, companias(nombre, codigo), asegurados(nombre, apellidos, telefono)', { count: 'exact' })
...
asignacionesRoutes.get('/historial', async (c) => {
    ...
    let query = supabase
        ...
        .select(`*, expedientes(numero_expediente, estado, compania_id, companias(nombre)),
       tramitador_anterior:tramitadores!tramitador_anterior_id(nombre, apellidos),
       tramitador_nuevo:tramitadores!tramitador_nuevo_id(nombre, apellidos)`, { count: 'exact' });
    if (tramitadorId) {
...
asignacionesRoutes.get('/reglas-reparto', async (c) => {
    ...
    if (empresaId)
        query = query.or(`empresa_facturadora_id.eq.${empresaId},empresa_facturadora_id.is.null`);
...
asignacionesRoutes.post('/reglas-reparto/:id/test', async (c) => {
    ...
    const { data: cola } = await supabase
        ...
        .not('estado', 'in', '(CERRADO,CANCELADO,COBRADO,FACTURADO)')
        .limit(10);
    const preview = await Promise.all((cola ?? []).map(async (exp) => {
