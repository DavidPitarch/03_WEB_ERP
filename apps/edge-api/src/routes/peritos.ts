...
export const peritosRoutes = new Hono<{ Bindings: Env }>();

function err(code: string, message: string) {
  return { data: null, error: { code, message } };
}

function ensurePeritoAdmin(c: any) {
...
async function getCurrentPerito(c: any) {
...
peritosRoutes.get('/mis-expedientes', async (c) => {
  ...
  if (!perito) return c.json(err('NOT_PERITO', 'Usuario no es perito activo'), 403);
  ...
  if (fecha_desde) query = query.gte('fecha_encargo', fecha_desde);
  if (fecha_hasta) query = query.lte('fecha_encargo', fecha_hasta);
...
peritosRoutes.get('/expedientes/:id', async (c) => {
  ...
  if (error || !data) return c.json(err('NOT_FOUND', 'Expediente no encontrado o no asignado'), 404);
...
peritosRoutes.get('/dictamenes', async (c) => {
  ...
  if (estado) query = query.eq('estado', estado);
  if (fecha_desde) query = query.gte('fecha_inspeccion', fecha_desde);
  if (fecha_hasta) query = query.lte('fecha_inspeccion', fecha_hasta);
...
peritosRoutes.post('/dictamenes', async (c) => {
  ...
  if (!exp) return c.json(err('NOT_ASSIGNED', 'Perito no asignado a este expediente'), 403);
...
peritosRoutes.put('/dictamenes/:id', async (c) => {
  ...
  if (!existing) return c.json(err('NOT_FOUND', 'Dictamen no encontrado'), 404);
  if (existing.estado !== 'borrador') return c.json(err('ESTADO_INVALIDO', 'Solo se pueden editar dictámenes en borrador'), 422);
...
peritosRoutes.post('/dictamenes/:id/emitir', async (c) => {
  ...
  if (dictamen.estado !== 'borrador') return c.json(err('ESTADO_INVALIDO', 'Solo se pueden emitir dictámenes en borrador'), 422);
...
peritosRoutes.post('/dictamenes/:id/evidencias', async (c) => {
  ...
  const body = await c.req.json<{
    ...
    clasificacion: 'dano' | 'causa' | 'contexto' | 'detalle';
    notas?: string;
  }>();
...
peritosRoutes.put('/:id', async (c) => {
  ...
  if (error || !data) return c.json(err('NOT_FOUND', 'Perito no encontrado'), 404);
...
peritosRoutes.put('/asignar-expediente/:expedienteId', async (c) => {
  ...
  const { perito_id } = await c.req.json<{ perito_id: string }>();
  ...
  if (error || !data) return c.json(err('NOT_FOUND', 'Expediente no encontrado'), 404);
