...
export const vpWebhooksRoutes = new Hono<{ Bindings: Env }>();
...
async function findSession(supabase: SupabaseClient, externalSessionId: string) {
...
async function getVpExpediente(supabase: SupabaseClient, vpId: string) {
  const { data } = await supabase
...
async function insertTimelineAndEvent(
...
vpWebhooksRoutes.post('/', async (c) => {
  ...
  const eventType = c.req.header('x-vp-event-type') as EventType;
  ...
  let videoperitacionId: string | null = null;
    ...
    switch (eventType) {
      ...
      case 'session.created': {
        ...
        const { data: sesion, error: sesErr } = await supabase
          ...
          .select('id')
          .single();
        if (sesErr) throw new Error(`Insert vp_sesiones: ${sesErr.message}`);
        ...
        if (vp) {
          await insertTimelineAndEvent(
            supabase, vp.expediente_id, correlation_id, eventType,
            `Sesión de videoperitación creada. Sala: ${room_url}`, SYSTEM_ACTOR,
          );
        }
      ...
      case 'session.started': {
        ...
        if (vp) {
          await insertTimelineAndEvent(
            supabase, vp.expediente_id, vp.id, eventType,
            `Sesión de videoperitación iniciada con ${Array.isArray(participants) ? participants.length : 0} participante(s).`,
            SYSTEM_ACTOR,
          );
        }
      ...
      case 'session.ended': {
        ...
        if (vp) {
          await insertTimelineAndEvent(
            supabase, vp.expediente_id, vp.id, eventType,
            `Sesión de videoperitación finalizada. Duración: ${duration_seconds}s, Participantes: ${participants_count}.`,
            SYSTEM_ACTOR,
          );
        }
      ...
      case 'recording.ready': {
        ...
        const { data: artefacto, error: artErr } = await supabase
          ...
          .select('id')
          .single();
        if (artErr) throw new Error(`Insert vp_artefactos: ${artErr.message}`);

        const vp = await getVpExpediente(supabase, session.videoperitacion_id);
        if (vp) {
          await insertTimelineAndEvent(
            supabase, vp.expediente_id, vp.id, eventType,
            `Grabación de videoperitación lista. Formato: ${format}, Duración: ${duration_seconds}s.`,
            SYSTEM_ACTOR,
          );
        }
      ...
      case 'audio.ready': {
        ...
        if (vp) {
          await insertTimelineAndEvent(
            supabase, vp.expediente_id, vp.id, eventType,
            `Audio de videoperitación listo. Formato: ${format}, Duración: ${duration_seconds}s.`,
            SYSTEM_ACTOR,
          );
        }
      ...
      case 'transcript.ready': {
        ...
        if (trErr) throw new Error(`Insert vp_transcripciones: ${trErr.message}`);

        const vp = await getVpExpediente(supabase, session.videoperitacion_id);
        if (vp) {
          await insertTimelineAndEvent(
            supabase, vp.expediente_id, vp.id, eventType,
            `Transcripción de videoperitación lista. Idioma: ${language}.${summary ? ` Resumen: ${summary.slice(0, 200)}` : ''}`,
            SYSTEM_ACTOR,
          );
        }
      ...
      case 'session.failed': {
        ...
        if (vp) {
          await insertTimelineAndEvent(
            supabase, vp.expediente_id, vp.id, eventType,
            `Sesión de videoperitación fallida. Motivo: ${reason}.`,
            SYSTEM_ACTOR,
          );
        }
      ...
      case 'participant.absent': {
        ...
        if (vp) {
          await insertTimelineAndEvent(
            supabase, vp.expediente_id, vp.id, eventType,
            `Participante ausente (${absent_role}) en videoperitación. Esperó ${waited_seconds}s.`,
            SYSTEM_ACTOR,
          );
        }
