...
describe('sendCustomerTrackingEmail — escenarios de estado de envío', () => {
  ...
  it('[dry_run] opera sin RESEND_API_KEY y devuelve success=true, dryRun=true', async () => {
    ...
    const result = await sendCustomerTrackingEmail(
      supabase,
...
describe('sendCustomerTrackingEmail — contenido del template HTML', () => {
  ...
  it('el HTML incluye el enlace en texto plano además del botón CTA', async () => {
    ...
    await sendCustomerTrackingEmail(
      supabase, 'key',
      { tokenId: 't', to: 'a@b.com', aseguradoNombre: 'A', numeroExpediente: 'EXP-1',
        companiaLabel: null, trackingUrl, expiresAt: new Date().toISOString() },
      'u',
    );

    const html = JSON.parse((fetchMock.mock.calls[0][1] as RequestInit).body as string).html;
