...
// ─── Envío de enlace de seguimiento al cliente (EP-12 Sprint 1) ───
//
// Wrapper de alto nivel para el portal de customer tracking.
// Persiste el resultado del envío en customer_tracking_tokens y emite
// el domain event CustomerTrackingEmailEnviado.
//
// Flujo degradado:
//   - Sin RESEND_API_KEY → dry_run (log, sin llamada real)
//   - Con key pero fallo → email_status = 'failed' + email_error con detalle
//   - Sin email del cliente → el caller debe pasar email_status = 'no_email'
//     y no llamar a esta función (ver customer-tracking.ts)

export interface CustomerTrackingEmailParams {
  tokenId: string;
  to: string;
  aseguradoNombre: string;
  numeroExpediente: string;
  companiaLabel: string | null;
  trackingUrl: string;
  expiresAt: string;
}
...
export async function sendCustomerTrackingEmail(
  supabase: SupabaseClient,
  apiKey: string | undefined,
  params: CustomerTrackingEmailParams,
  actorId: string,
): Promise<SendResult> {
