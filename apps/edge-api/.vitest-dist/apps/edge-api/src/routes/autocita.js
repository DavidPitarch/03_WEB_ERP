...
import { buildCustomerTrackingActionMetadata, buildAuthorizedTechnicianLabel, } from '../services/customer-tracking';
import { consumeAutocitaToken, countCambiosExpediente, findAutocitaTokenByRawValue, hashAutocitaToken, insertAutocitaSeleccion, resolveAutocitaConfig, validateAutocitaToken, } from '../services/autocita-tokens';
import { computeAvailableSlots, parseSlotId, resolveFranjaFin, validateSlotStillAvailable, } from '../services/autocita-engine';
...
export const autocitaPublicRoutes = new Hono();
export const autocitaAdminRoutes = new Hono();
...
autocitaPublicRoutes.get('/:token', async (c) => {
    ...
    const operario = expediente.operarios ?? null;
    const now = new Date();
    ...
    const slaFechaLimite = expediente.fecha_limite_sla ?? null;
    const view = {
        expediente: {
...
autocitaPublicRoutes.get('/:token/slots', async (c) => {
    ...
    if (!expediente?.operario_id) {
        return c.json({ data: { slots: [], total: 0, cambios_restantes: 0, mensaje_sin_huecos: 'No hay operario asignado al expediente.' }, error: null });
    }
    const companiaConfig = expediente.companias?.config ?? null;
    const cfg = resolveAutocitaConfig(companiaConfig);
    const cambiosRestantes = Math.max(0, cfg.maxCambiosPorExpediente - cambiosUsados);
    ...
    const { data: operario } = await supabase
        ...
        .single();
    if (!operario) {
        return c.json({ data: { slots: [], total: 0, cambios_restantes: cambiosRestantes, mensaje_sin_huecos: 'No hay operario disponible.' }, error: null });
    }
    // Resolve comunidad from provincia (simplified mapping for SLA check)
    const provinciaComunidadMap = {
...
async function confirmCita(supabase, cita, token, ip, userAgent, c) {
...
autocitaPublicRoutes.post('/:token/seleccionar', async (c) => {
    ...
    if (!schemaResult.ok)
        return validationError(c, schemaResult.errors);
    // Check cambios limit
    const { data: expediente } = await supabase
        .from('expedientes')
