...
tareasRoutes.get('/metricas', async (c) => {
    ...
    const en_progreso = todas.filter((t) => t.estado === 'en_progreso').length;
    const pospuestas = todas.filter((t) => t.estado === 'pospuesta').length;
    const resueltas = todas.filter((t) => t.estado === 'resuelta').length;
    const vencidas = todas.filter((t) => t.fecha_limite && t.fecha_limite < now && !['resuelta', 'cancelada'].includes(t.estado)).length;
    // Tiempo medio de resolucion en las ultimas 30 dias
    const resueltasRecientes = todas.filter((t) => t.estado === 'resuelta' && t.resuelta_at && t.resuelta_at >= thirtyDaysAgo);
    let tiempo_medio_resolucion_horas = null;
    if (resueltasRecientes.length > 0) {
        const totalMs = resueltasRecientes.reduce((sum, t) => {
