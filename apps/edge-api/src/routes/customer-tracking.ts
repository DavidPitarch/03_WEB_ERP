...
import type {
  ...
  CustomerTrackingIssueLinkRequest,
  CustomerTrackingIssueLinkResponse,
...
import {
  ...
  buildCustomerTrackingContact,
  buildCustomerTrackingTimeline,
  buildCustomerTrackingView,
  canConfirmCustomerAppointment,
  canRequestCustomerReschedule,
...
async function insertAccessLog(
  ...
  params: {
    ...
    citaId?: string | null;
    action: 'emitir_link' | 'view' | 'confirmar_cita' | 'solicitar_cambio';
    ok: boolean;
    ip: string | null;
...
async function findTokenByRawValue(
  supabase: SupabaseClient,
  token: string,
): Promise<TrackingTokenRow | null> {
  ...
  return (data as TrackingTokenRow | null) ?? null;
}

async function resolveCurrentCita(
  supabase: SupabaseClient,
  expedienteId: string,
) {
...
async function resolveCustomerTrackingContext(
  supabase: SupabaseClient,
  expedienteId: string,
) {
  ...
  const timeline = buildCustomerTrackingTimeline({
    ...
    citas: citas.data ?? [],
    comunicaciones: (comunicaciones.data as Array<{ id: string; contenido: string; metadata?: Record<string, unknown> | null; created_at: string }>) ?? [],
  });
  ...
  return {
    expediente,
    cita,
    operario: (expediente as any).operarios ?? null,
    contacto,
    timeline,
  };
}
