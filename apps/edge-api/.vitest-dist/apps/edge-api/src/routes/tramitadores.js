...
export const tramitadoresRoutes = new Hono();
...
tramitadoresRoutes.get('/', async (c) => {
    ...
    if (empresaId)
        query = query.eq('empresa_facturadora_id', empresaId);
    if (activo !== undefined)
...
tramitadoresRoutes.get('/dashboard', async (c) => {
    ...
    if (empresaId)
        sinAsignarQuery = sinAsignarQuery.eq('empresa_facturadora_id', empresaId);
    const { count: sinAsignar } = await sinAsignarQuery;
    const kpis = {
        total_tramitadores_activos: lista.length,
        total_activos: lista.reduce((s, t) => s + (t.total_activos ?? 0), 0),
        tramitadores_sobrecargados: lista.filter((t) => t.semaforo === 'rojo').length,
        tramitadores_en_alerta: lista.filter((t) => t.semaforo === 'amarillo').length,
        total_sla_vencidos: lista.reduce((s, t) => s + (t.total_sla_vencidos ?? 0), 0),
        total_sin_cita: lista.reduce((s, t) => s + (t.total_sin_cita ?? 0), 0),
        total_bloqueados: lista.reduce((s, t) => s + (t.total_bloqueados ?? 0), 0),
        expedientes_sin_tramitador: sinAsignar ?? 0,
        carga_promedio_pct: lista.length > 0
            ? Math.round(lista.reduce((s, t) => s + (t.porcentaje_carga ?? 0), 0) / lista.length)
            : 0,
        tramitadores: lista,
    };
...
tramitadoresRoutes.put('/:id/capacidad', async (c) => {
    ...
    const body = await c.req.json();
    const patch = {};
    if (body.max_expedientes_activos !== undefined)
        patch.max_expedientes_activos = body.max_expedientes_activos;
    if (body.max_urgentes !== undefined)
        patch.max_urgentes = body.max_urgentes;
    if (body.max_por_compania !== undefined)
        patch.max_por_compania = body.max_por_compania;
    if (body.umbral_alerta_pct !== undefined)
        patch.umbral_alerta_pct = body.umbral_alerta_pct;
    if (Object.keys(patch).length === 0) {
...
tramitadoresRoutes.get('/:id/expedientes', async (c) => {
    ...
    let query = supabase
        .from('expedientes')
        .select('*, companias(nombre, codigo), asegurados(nombre, apellidos)', { count: 'exact' })
        .eq('tramitador_id', id)
...
tramitadoresRoutes.get('/:id/historial', async (c) => {
    ...
    const { data, error, count } = await supabase
       ...
       actor:user_profiles!actor_id(nombre, apellidos)`, { count: 'exact' })
...
tramitadoresRoutes.put('/:tramitadorId/preasignaciones/:ruleId', async (c) => {
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error || !data)
        return c.json({ data: null, error: { code: 'NOT_FOUND', message: 'Regla no encontrada' } }, 404);
    return c.json({ data, error: null });
});
