...
describe('Production Gate — CORS dynamic origins', () => {
  ...
  const parseAllowedOrigins = (env?: string): string[] => {
...
describe('Production Gate — Rate limiter', () => {
  ...
  it('rejects after limit is reached', () => {
    const limiter = createLimiter();
    for (let i = 0; i < RATE_LIMIT; i++) limiter.check('10.0.0.1');
  ...
  it('resets after window expires', () => {
    const limiter = createLimiter();
    const now = Date.now();
    for (let i = 0; i < RATE_LIMIT; i++) limiter.check('10.0.0.1', now);
...
describe('Production Gate — Public confirm endpoint', () => {
  ...
  const validate = (token: string, rows: ConfirmRow[], now = new Date()) => {
    if (!token) return { ok: false, error: 'missing_token' };
    const row = rows.find(r => r.token === token);
    if (!row) return { ok: false, error: 'invalid_token' };
    if (row.confirmed_at) return { ok: false, error: 'already_confirmed' };
    if (new Date(row.expires_at) < now) return { ok: false, error: 'token_expired' };
    return { ok: true, pedido_id: row.id };
  };

  const validRow: ConfirmRow = {
    id: 'ped-001',
    token: 'abc123',
...
describe('Production Gate — Scheduled Worker (4 cron jobs)', () => {
  ...
  it('job 1: detecta pedidos caducados', () => {
    ...
    const caducados = pedidos.filter(p =>
      ['pendiente', 'enviado'].includes(p.estado) && new Date(p.fecha_limite) < now
    );
  ...
  it('job 2: detecta facturas vencidas', () => {
    ...
    const vencidas = facturas.filter(f =>
      ['emitida', 'enviada'].includes(f.estado) &&
...
describe('Production Gate — Signed URLs', () => {
  const buildSignedPath = (bucket: string, expedienteId: string, filename: string) => {
    const ts = Date.now();
    return `${bucket}/${expedienteId}/${ts}-${filename}`;
  };

  const initUpload = (expedienteId: string, filename: string) => {
...
describe('Production Gate — RLS E2E role matrix', () => {
  ...
  const canAccess = (role: string | null, resource: string): boolean => {
    if (role === null) return PUBLIC_PATHS.some(p => resource.startsWith(p));
    if (resource === 'facturas') return FACTURA_ROLES.includes(role);
    if (resource === 'pedidos') return PEDIDO_ROLES.includes(role);
    if (resource === 'cobro') return COBRO_ROLES.includes(role);
    if (resource === 'autofacturas') return AUTOFACTURA_ROLES.includes(role);
    return false;
  };
