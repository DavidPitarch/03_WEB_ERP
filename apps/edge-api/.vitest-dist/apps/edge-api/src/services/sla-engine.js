 ...
 */
function countBusinessDays(desde, hasta, festivoSet) {
    let count = 0;
 ...
 */
function calendarMsToBusinessMs(startDate, calendarMs, festivoSet) {
    if (calendarMs <= 0)
        return 0;
 ...
 */
export async function calculateSlaStatus(supabase, expedienteId) {
    const now = new Date();
    // 1. Fetch expediente
    ...
    let estadoSla = 'sin_sla';
    if (fechaLimiteSla) {
        const slaLimitMs = calendarMsToBusinessMs(fechaEncargo, fechaLimiteSla.getTime() - fechaEncargo.getTime(), festivoSet);
        if (slaLimitMs > 0) {
 ...
 */
export async function registerSlaPause(supabase, expedienteId, estadoPausa, motivo) {
    const { data, error } = await supabase
        ...
        .single();
    if (error)
        throw new Error(`Error registrando pausa SLA: ${error.message}`);
    return { id: data.id };
}
/**
 * Close any open SLA pause for an expediente (sets fin = NOW()).
 */
export async function closeSlaPause(supabase, expedienteId) {
    const { data, error } = await supabase
        .from('sla_pausas')
        ...
        .select('id');
    if (error)
        throw new Error(`Error cerrando pausa SLA: ${error.message}`);
 ...
 */
export async function getCalendarioLaboral(supabase, desde, hasta) {
    const { data, error } = await supabase
