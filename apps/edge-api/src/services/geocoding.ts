...
async function saveCache(
  supabase: SupabaseClient,
  raw: string,
  point: Omit<GeoPoint, 'source'>
): Promise<void> {
  await supabase.from('geo_cache').upsert(
    { direccion_raw: raw, lat: point.lat, lng: point.lng, confidence: point.confidence, source: 'nominatim' },
    { onConflict: 'direccion_raw' }
  );
}
...
export async function geocodeExpediente(
  supabase: SupabaseClient,
  expediente: {
    id: string;
