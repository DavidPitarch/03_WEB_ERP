...
function canEmitFactura(role: string): boolean {
  return FACTURA_ROLES.includes(role);
}

function canEnviarInforme(role: string): boolean {
  return ENVIO_ROLES.includes(role);
}

function canGenerateDocFinal(role: string): boolean {
  return DOCFINAL_GENERATE_ROLES.includes(role);
}

function canRegenerateDocFinal(role: string): boolean {
...
interface Valoracion {
  ...
  baremo_id: string;
  baremo_version: number;
  baremo_nombre: string;
}
...
interface VP {
  ...
  documento_final_version?: number;
}
...
interface Factura {
  ...
  empresa_cif: string;
  total: number;
  lineas: Array<{ descripcion: string; cantidad: number; precio_unitario: number; importe: number }>;
  created_at: string;
}
...
function formatNumeroFactura(prefijo: string, year: number, counter: number): string {
  return `${prefijo}${year}-${String(counter).padStart(5, '0')}`;
}

function checkFacturaPreconditions(vp: VP, empresaCif: string | null): { ok: boolean; code: number; error?: string } {
...
function checkEnvioPreconditions(vp: VP): { ok: boolean; code: number; error?: string } {
...
function makeVP(overrides: Partial<VP> = {}): VP {
  return {
    id: 'vp-1',
...
function makeEnvio(overrides: Partial<Envio> = {}): Envio {
  return {
    id: 'envio-1',
    vp_id: 'vp-1',
    canal: 'email',
