...
export const planningGeoRoutes = new Hono();
...
planningGeoRoutes.get('/geo/expedientes', async (c) => {
    ...
    if (prioridad)
        query = query.in('prioridad', prioridad.split(','));
    if (fechaIni || fechaFin) {
        if (fechaIni)
            query = query.gte('fecha_encargo', fechaIni);
        if (fechaFin)
            query = query.lte('fecha_encargo', fechaFin);
    }
    if (bbox) {
    ...
    if (gremio) {
        ...
        items = items.filter((e) => e.operario_id && opIds.has(e.operario_id));
    }
    // Contadores resumen
    const unassigned = items.filter((e) => ['NUEVO', 'NO_ASIGNADO'].includes(e.estado)).length;
    const alerts = items.filter((e) => e.sla_status === 'vencido' || e.sla_status === 'urgente').length;
    return c.json({
        data: items,
        meta: {
            total: items.length,
...
planningGeoRoutes.get('/geo/operarios', async (c) => {
    ...
    if (gremio)
        query = query.contains('gremios', [gremio]);
