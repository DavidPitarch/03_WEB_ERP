...
import type { AutocitaSlot } from '@erp/types';
...
const FRANJAS_DEFAULT: Array<{ inicio: string; fin: string }> = [
...
function isDayOfWeek(date: Date, allowedDays: number[]): boolean {
...
async function checkOperarioDisponible(
  ...
  return Boolean((data as { disponible?: boolean }).disponible);
}

/** Returns the count of non-cancelled citas for an operario on a given date+franja. */
async function countCitasEnFranja(
  supabase: SupabaseClient,
  operarioId: string,
  fecha: string,
  franjaInicio: string,
...
export interface ComputeAvailableSlotsParams {
  supabase: SupabaseClient;
  operarioId: string;
  operarioZonasCp: string[];
  operarioGremios: string[];
  expedienteCodigoPostal: string;
  expedienteTipoSiniestro: string;
  expedienteEmpresaId: string | null;
  expedienteProvincia: string | null;
  expedienteComunidad: string | null;
  slaFechaLimite: string | null;
  maxSlots: number;
  diasMaxSeleccion: number;
  margenAvisoH: number;
  bufferSlaH: number;
}
...
export async function computeAvailableSlots(
...
): Promise<AutocitaSlot[]> {
  const {
    supabase,
    operarioId,
    operarioZonasCp,
    operarioGremios,
    expedienteCodigoPostal,
    expedienteTipoSiniestro,
    slaFechaLimite,
    maxSlots,
    diasMaxSeleccion,
    margenAvisoH,
  ...
  const gremioMatch = operarioGremios.length === 0 || operarioGremios.some((g) =>
    expedienteTipoSiniestro.toLowerCase().includes(g.toLowerCase()) ||
    g.toLowerCase().includes(expedienteTipoSiniestro.toLowerCase()),
  );

  if (!gremioMatch) return [];
  ...
  const windowEnd = new Date(Math.min(
    addDays(now, diasMaxSeleccion).getTime(),
    maxEnd.getTime(),
  ));

  const slots: AutocitaSlot[] = [];
  ...
  while (cursor <= windowEnd && slots.length < maxSlots) {
    ...
    for (const franja of FRANJAS_DEFAULT) {
      ...
      if (franjaDateTime < minStart) continue;

      // Skip if franja start is after the SLA window
      if (franjaDateTime > windowEnd) continue;
      ...
      if (citasCount >= MAX_CITAS_POR_FRANJA) continue;
...
export async function validateSlotStillAvailable(
  supabase: SupabaseClient,
  params: {
    operarioId: string;
    fecha: string;
  ...
  if (!disponible) return { ok: false, reason: 'OPERARIO_NO_DISPONIBLE' };

  const count = await countCitasEnFranja(supabase, params.operarioId, params.fecha, params.franjaInicio, params.franjaFin);
  if (count >= MAX_CITAS_POR_FRANJA) return { ok: false, reason: 'SLOT_LLENO' };
...
export function parseSlotId(slotId: string): {
  ...
  const uuidLen = 36;
  if (slotId.length < uuidLen + 1 + 10 + 1 + 5) return null;
