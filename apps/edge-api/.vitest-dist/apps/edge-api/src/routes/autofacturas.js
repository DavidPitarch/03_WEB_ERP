...
export const autofacturasRoutes = new Hono();
// ─── Helper ────────────────────────────────────────────────────────────────
...
autofacturasRoutes.get('/liquidables', async (c) => {
    ...
    if (periodo_desde)
        query = query.gte('periodo_desde', periodo_desde);
    if (periodo_hasta)
        query = query.lte('periodo_hasta', periodo_hasta);
    ...
    if (error) {
        ...
        if (operario_id)
            opQuery = opQuery.eq('id', operario_id);
        const { data: operarios, error: opErr } = await opQuery;
        if (opErr)
            return c.json(err('DB_ERROR', opErr.message), 500);
        const results = [];
        for (const op of operarios ?? []) {
            ...
            if (periodo_desde)
                partesQuery = partesQuery.gte('fecha', periodo_desde);
            if (periodo_hasta)
                partesQuery = partesQuery.lte('fecha', periodo_hasta);
            ...
            if (partes && partes.length > 0) {
                ...
                const usadoIds = new Set((usados ?? []).map((u) => u.parte_id));
                const pendientes = parteIds.filter((id) => !usadoIds.has(id));
...
autofacturasRoutes.post('/generar', async (c) => {
    ...
    const { data: operario } = await supabase
        ...
        .single();
    if (!operario)
        return c.json(err('NOT_FOUND', 'Operario no encontrado'), 404);
    ...
    if (partesErr)
        return c.json(err('DB_ERROR', partesErr.message), 500);
    if (!partes || partes.length === 0) {
    ...
    const { data: usados } = await supabase
        ...
        .in('parte_id', parteIds);
    const usadoIds = new Set((usados ?? []).map((u) => u.parte_id));
    const partesNuevos = partes.filter((p) => !usadoIds.has(p.id));
    ...
    const parteNuevoIds = partesNuevos.map((p) => p.id);
    ...
    const { data: autofactura, error: afErr } = await supabase
        ...
        .single();
    if (afErr)
        return c.json(err('DB_ERROR', afErr.message), 500);
    // Insert lineas
    if (lineas.length > 0) {
...
autofacturasRoutes.get('/:id', async (c) => {
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error || !data)
        return c.json(err('NOT_FOUND', 'Autofactura no encontrada'), 404);
    return c.json({ data, error: null });
});
// ═══════════════════════════════════════════════════════════════════════════
