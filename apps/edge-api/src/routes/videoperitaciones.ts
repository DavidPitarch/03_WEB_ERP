...
import {
  assertVpArtefactAccess,
  VP_SIGNED_URL_TTL_SECONDS,
  VpArtefactAccessError,
} from '../services/vp-access';
...
videoperitacionesRoutes.get('/pendientes-contacto', async (c) => {
  ...
  if (perito_id) query = query.eq('perito_id', perito_id);
...
videoperitacionesRoutes.get('/:id', async (c) => {
  ...
  if (error || !vp) return c.json(err('NOT_FOUND', 'Videoperitación no encontrada'), 404);
...
videoperitacionesRoutes.post('/', async (c) => {
  ...
  if (!body.expediente_id) return c.json(err('VALIDATION', 'expediente_id es requerido'), 422);
  ...
  if (vpErr) return c.json(err('DB_ERROR', vpErr.message), 500);
...
videoperitacionesRoutes.post('/:id/registrar-encargo', async (c) => {
  ...
  if (!body.tipo || !body.contenido) return c.json(err('VALIDATION', 'tipo y contenido son requeridos'), 422);
  ...
  if (!vp) return c.json(err('NOT_FOUND', 'Videoperitación no encontrada'), 404);
  ...
  if (encargoErr) return c.json(err('DB_ERROR', encargoErr.message), 500);
...
videoperitacionesRoutes.post('/:id/registrar-comunicacion', async (c) => {
  ...
  if (comErr) return c.json(err('DB_ERROR', comErr.message), 500);
...
videoperitacionesRoutes.post('/:id/registrar-intento-contacto', async (c) => {
  ...
  if (!body.canal || !body.resultado) return c.json(err('VALIDATION', 'canal y resultado son requeridos'), 422);
  ...
  if (intentoErr) return c.json(err('DB_ERROR', intentoErr.message), 500);
...
videoperitacionesRoutes.post('/:id/agendar', async (c) => {
  ...
  if (agendaErr) return c.json(err('DB_ERROR', agendaErr.message), 500);
...
videoperitacionesRoutes.post('/:id/reprogramar', async (c) => {
  ...
  if (updateErr) return c.json(err('DB_ERROR', updateErr.message), 500);
...
videoperitacionesRoutes.post('/:id/cancelar', async (c) => {
  ...
  if (!motivo) return c.json(err('VALIDATION', 'motivo es requerido'), 422);
...
videoperitacionesRoutes.post('/:id/enviar-link', async (c) => {
  ...
  if (!body.link_externo) return c.json(err('VALIDATION', 'link_externo es requerido'), 422);
  ...
  if (!agenda) return c.json(err('NO_AGENDA', 'No hay cita activa para esta videoperitación'), 422);
...
videoperitacionesRoutes.get('/:id/transcripciones/:transcripcionId', async (c) => {
  ...
  if (error || !data) return c.json(err('NOT_FOUND', 'Transcripción no encontrada'), 404);
  return c.json({ data, error: null });
});
...
videoperitacionesRoutes.get('/buscar-transcripcion', async (c) => {
  ...
  if (vpId) query = query.eq('videoperitacion_id', vpId);
...
videoperitacionesRoutes.get('/:id/dictamenes/:did', async (c) => {
  ...
  const { data, error } = await supabase
    ...
    .eq('id', did)
    .single();
  if (error || !data) return c.json(err('NOT_FOUND', 'Dictamen no encontrado'), 404);
  return c.json({ data, error: null });
});
...
videoperitacionesRoutes.post('/:id/dictamenes', async (c) => {
  ...
  if (dErr) return c.json(err('DB_ERROR', dErr.message), 500);
...
videoperitacionesRoutes.post('/:id/emitir-dictamen', async (c) => {
  ...
  if (!body.dictamen_id) return c.json(err('VALIDATION', 'dictamen_id es requerido'), 422);
  if (!body.tipo_resolucion) return c.json(err('VALIDATION', 'tipo_resolucion es requerido'), 422);
  if (!body.conclusiones) return c.json(err('VALIDATION', 'conclusiones son requeridas'), 422);
  ...
  if (!dictamen) return c.json(err('NOT_FOUND', 'Dictamen no encontrado'), 404);
  ...
  if (uErr) return c.json(err('DB_ERROR', uErr.message), 500);
...
videoperitacionesRoutes.post('/:id/solicitar-mas-informacion', async (c) => {
  ...
  if (!body.informacion_solicitada) return c.json(err('VALIDATION', 'informacion_solicitada es requerida'), 422);

  const { data: vp } = await supabase.from('vp_videoperitaciones')
    .select('id, expediente_id, perito_id').eq('id', id).single();
  if (!vp) return c.json(err('NOT_FOUND', 'VP no encontrada'), 404);
...
videoperitacionesRoutes.post('/:id/rechazar', async (c) => {
  ...
  if (!body.motivo_rechazo) return c.json(err('VALIDATION', 'motivo_rechazo es requerido'), 422);
...
videoperitacionesRoutes.post('/:id/instruccion', async (c) => {
  ...
  if (!body.tipo || !body.descripcion) return c.json(err('VALIDATION', 'tipo y descripcion son requeridos'), 422);
  ...
  if (iErr) return c.json(err('DB_ERROR', iErr.message), 500);
...
videoperitacionesRoutes.post('/:id/rechazar-dictamen', async (c) => {
  ...
  if (!body.dictamen_id || !body.motivo) return c.json(err('VALIDATION', 'dictamen_id y motivo son requeridos'), 422);
...
videoperitacionesRoutes.get('/:id/informes/:iid', async (c) => {
  ...
  if (error || !informe) return c.json({ data: null, error: { code: 'NOT_FOUND', message: 'Informe no encontrado' } }, 404);
...
videoperitacionesRoutes.post('/:id/informes', async (c) => {
  ...
  const isPerito = user.roles?.some((r: string) => ['perito', 'admin', 'supervisor'].includes(r));
  if (!isPerito) return c.json({ data: null, error: { code: 'FORBIDDEN', message: 'Solo perito/admin/supervisor pueden crear informes' } }, 403);
  ...
  if (vpErr || !vp) return c.json({ data: null, error: { code: 'NOT_FOUND', message: 'VP no encontrada' } }, 404);
  ...
  let hallazgos: any[] = [];
  let resolucion_pericial: Record<string, any> = {};
  let dictamen_id: string | null = null;
  ...
  if (insertErr) return c.json({ data: null, error: { code: 'DB_ERROR', message: insertErr.message } }, 500);
...
videoperitacionesRoutes.post('/:id/informes/:iid/guardar-borrador', async (c) => {
  ...
  if (fetchErr || !informe) return c.json({ data: null, error: { code: 'NOT_FOUND', message: 'Informe no encontrado' } }, 404);
  ...
  const isOwnerOrAdmin = informe.creado_por === user.id || user.roles?.some((r: string) => ['admin', 'supervisor'].includes(r));
  if (!isOwnerOrAdmin) return c.json({ data: null, error: { code: 'FORBIDDEN', message: 'No autorizado' } }, 403);
  ...
  // Update content
  const updates: Record<string, any> = { version: informe.version + 1 };
  if (body.conclusiones !== undefined) updates.conclusiones = body.conclusiones;
  if (body.hallazgos !== undefined) updates.hallazgos = body.hallazgos;
  if (body.observaciones_finales !== undefined) updates.observaciones_finales = body.observaciones_finales;
  if (body.evidencias_principales !== undefined) updates.evidencias_principales = body.evidencias_principales;
  if (body.extractos_transcripcion !== undefined) updates.extractos_transcripcion = body.extractos_transcripcion;
  if (body.resumen_sesion !== undefined) updates.resumen_sesion = body.resumen_sesion;

  const { data: updated, error: updateErr } = await supabase
    .from('vp_informes').update(updates).eq('id', iid).select().single();

  if (updateErr) return c.json({ data: null, error: { code: 'DB_ERROR', message: updateErr.message } }, 500);
...
videoperitacionesRoutes.post('/:id/informes/:iid/validar', async (c) => {
  ...
  const isAdminSup = user.roles?.some((r: string) => ['admin', 'supervisor'].includes(r));
  if (!isAdminSup) return c.json({ data: null, error: { code: 'FORBIDDEN', message: 'Solo admin/supervisor pueden validar informes' } }, 403);
...
videoperitacionesRoutes.post('/:id/informes/:iid/rectificar', async (c) => {
  ...
  if (!body.motivo?.trim()) return c.json({ data: null, error: { code: 'VALIDATION', message: 'motivo requerido' } }, 422);
...
videoperitacionesRoutes.get('/:id/valoracion', async (c) => {
  ...
  if (!valoracion) return c.json({ data: null, error: null });
...
videoperitacionesRoutes.post('/:id/calcular-valoracion', async (c) => {
  ...
  const body = await c.req.json<{
    lineas: Array<{ partida_baremo_id: string; cantidad: number; observaciones?: string }>;
  }>();

  if (!body.lineas?.length) return c.json({ data: null, error: { code: 'VALIDATION', message: 'Se requiere al menos una línea' } }, 422);
  ...
  if (!vp) return c.json({ data: null, error: { code: 'NOT_FOUND', message: 'VP no encontrada' } }, 404);

  const companiaId = vp.expedientes?.compania_id;
  if (!companiaId) return c.json({ data: null, error: { code: 'VALIDATION', message: 'Expediente sin compañía asignada' } }, 422);
  ...
  if (!baremo) return c.json({ data: null, error: { code: 'NO_BAREMO', message: 'No hay baremo vigente para esta compañía' } }, 422);
  ...
  const partidaMap = new Map((partidas ?? []).map((p: any) => [p.id, p]));
  ...
  // Insert lines
  if (lineas.length > 0) {
    await supabase.from('vp_valoracion_lineas').insert(
      lineas.map(l => ({ ...l, valoracion_id: valoracionId }))
    );
  }
...
videoperitacionesRoutes.post('/:id/valoracion/lineas', async (c) => {
  ...
  if (!valoracion) return c.json({ data: null, error: { code: 'NOT_FOUND', message: 'No existe valoración para esta VP' } }, 404);
  ...
  if (es_ajuste_manual) {
    const isAdminSup = user.roles?.some((r: string) => ['admin', 'supervisor'].includes(r));
    if (!isAdminSup) return c.json({ data: null, error: { code: 'FORBIDDEN', message: 'Ajuste manual solo para admin/supervisor' } }, 403);
    if (!body.motivo_ajuste?.trim()) return c.json({ data: null, error: { code: 'VALIDATION', message: 'motivo_ajuste requerido para ajustes manuales' } }, 422);
  }
...
videoperitacionesRoutes.post('/:id/valoracion/recalcular', async (c) => {
  ...
  if (!valoracion) return c.json({ data: null, error: { code: 'NOT_FOUND', message: 'No existe valoración' } }, 404);
...
videoperitacionesRoutes.post('/:id/informes/:iid/preview', async (c) => {
  ...
  if (!informe) return c.json({ data: null, error: { code: 'NOT_FOUND', message: 'Informe no encontrado' } }, 404);
...
videoperitacionesRoutes.post('/:id/documento-final/generar', async (c) => {
  ...
  const isAllowed = user.roles?.some((r: string) => ['perito', 'admin', 'supervisor'].includes(r));
  ...
  if (!informe) return c.json({ data: null, error: { code: 'PRECONDITION', message: 'Se requiere informe validado' } }, 422);
  ...
  const contenido_json = {
    ...
    valoracion: valoracion ? {
      ...
      lineas: (valoracion.vp_valoracion_lineas ?? []).map((l: any) => ({
        codigo: l.codigo,
...
videoperitacionesRoutes.post('/:id/documento-final/regenerar', async (c) => {
  ...
  const isAllowed = user.roles?.some((r: string) => ['admin', 'supervisor'].includes(r));
  if (!isAllowed) return c.json({ data: null, error: { code: 'FORBIDDEN', message: 'Solo admin/supervisor pueden regenerar' } }, 403);
...
videoperitacionesRoutes.post('/:id/emitir-factura', async (c) => {
  ...
  const isFinance = user.roles?.some((r: string) => ['admin', 'supervisor', 'financiero'].includes(r));
  if (!isFinance) return c.json({ data: null, error: { code: 'FORBIDDEN', message: 'Solo admin/supervisor/financiero' } }, 403);
  ...
  if (!body.serie_id) return c.json({ data: null, error: { code: 'VALIDATION', message: 'serie_id requerido' } }, 422);
  ...
  if (!informeCount) return c.json({ data: null, error: { code: 'PRECONDITION', message: 'Requiere informe validado' } }, 422);
  ...
  if (!valoracion) return c.json({ data: null, error: { code: 'PRECONDITION', message: 'Requiere valoración calculada o validada' } }, 422);
  ...
  if (existingCount && existingCount > 0) return c.json({ data: null, error: { code: 'DUPLICATE', message: 'Ya existe factura VP para esta videoperitación' } }, 409);
  ...
  if (!empresa?.cif) return c.json({ data: null, error: { code: 'PRECONDITION', message: 'Empresa facturadora sin CIF' } }, 422);
  ...
  if (!serie) return c.json({ data: null, error: { code: 'NOT_FOUND', message: 'Serie no encontrada o inactiva' } }, 404);
  ...
  if (counterErr) return c.json({ data: null, error: { code: 'CONFLICT', message: 'Error de concurrencia en numeración' } }, 409);
  ...
  if (facturaErr) return c.json({ data: null, error: { code: 'DB_ERROR', message: facturaErr.message } }, 500);
...
videoperitacionesRoutes.post('/:id/enviar-informe', async (c) => {
  ...
  if (!doc) return c.json({ data: null, error: { code: 'PRECONDITION', message: 'Se requiere documento final generado' } }, 422);
