...
export const configEmisionRoutes = new Hono();
// ─── Helper ────────────────────────────────────────────────────────────────
function err(code, message) {
    return { data: null, error: { code, message } };
}
const TIPOS_DOC = ['factura', 'factura_simplificada', 'autofactura', 'abono', 'rectificativa'];
const TIPOS_TERCERO = ['compania', 'cliente_final', 'operario_autonomo', 'proveedor', 'grupo_empresa', 'cualquiera'];
const FLUJOS = ['expediente', 'videoperitacion', 'manual', 'subcontrata', 'cualquiera'];
...
configEmisionRoutes.post('/reglas-numeracion', async (c) => {
    ...
    if (!result.ok)
        return validationError(c, result.errors);
    const sb = c.get('supabase');
    const { data, error } = await sb.from('reglas_numeracion')
        ...
        .single();
    if (error?.code === '23505')
        return c.json(err('NOMBRE_DUPLICADO', 'Ya existe una regla con ese nombre para esta empresa'), 422);
    if (error)
        return c.json(err('DB_ERROR', error.message), 500);
...
configEmisionRoutes.get('/reglas-numeracion/:id', async (c) => {
    ...
    if (error || !data)
        return c.json(err('NOT_FOUND', 'Regla no encontrada'), 404);
    return c.json({ data, error: null });
});
...
configEmisionRoutes.get('/series', async (c) => {
    ...
    if (tipo_tercero)
        q = q.eq('tipo_tercero', tipo_tercero);
    if (ejercicio_fiscal)
        q = q.eq('ejercicio_fiscal', ejercicio_fiscal);
    if (activa !== undefined)
...
configEmisionRoutes.post('/series', async (c) => {
    ...
    if (error?.code === '23505')
        return c.json(err('SERIE_DUPLICADA', 'Ya existe una serie con esa combinación de código/empresa/tipo'), 422);
    if (error)
        return c.json(err('DB_ERROR', error.message), 500);
...
configEmisionRoutes.get('/series/resolver', async (c) => {
    ...
    if (error)
        return c.json(err('DB_ERROR', error.message), 500);
    if (!serieId)
        return c.json(err('SIN_SERIE_ASIGNADA', 'No existe una asignación de serie para este contexto'), 404);
...
configEmisionRoutes.get('/series/:id', async (c) => {
    const { data, error } = await c.get('supabase').from('series_facturacion')
        ...
        .single();
    if (error || !data)
        return c.json(err('NOT_FOUND', 'Serie no encontrada'), 404);
    return c.json({ data, error: null });
});
...
configEmisionRoutes.post('/series/:id/reset-contador', async (c) => {
    ...
    if (!hasAdminRole(user.roles))
        return forbiddenAdmin(c);
    const { nuevo_valor = 0, motivo } = await c.req.json();
    if (!motivo)
        return c.json(err('MOTIVO_REQUERIDO', 'Se requiere motivo para resetear el contador'), 422);
    const sb = c.get('supabase');
    const id = c.req.param('id');
...
configEmisionRoutes.post('/cuentas-bancarias', async (c) => {
    ...
    const { data, error } = await sb.from('cuentas_bancarias_empresa')
        ...
        .single();
    if (error?.code === '23505')
        return c.json(err('IBAN_DUPLICADO', 'Ese IBAN ya está registrado para esta empresa'), 422);
    if (error)
        return c.json(err('DB_ERROR', error.message), 500);
...
configEmisionRoutes.put('/cuentas-bancarias/:id', async (c) => {
    ...
    if (error)
        return c.json(err('DB_ERROR', error.message), 500);
    if (!data)
        return c.json(err('NOT_FOUND', 'Cuenta no encontrada'), 404);
...
configEmisionRoutes.post('/asignaciones', async (c) => {
    ...
    const { data, error } = await sb.from('asignaciones_serie')
        ...
        .single();
    if (error?.code === '23505')
        return c.json(err('ASIGNACION_DUPLICADA', 'Ya existe una asignación activa con los mismos criterios'), 422);
    if (error)
        return c.json(err('DB_ERROR', error.message), 500);
...
configEmisionRoutes.put('/asignaciones/:id', async (c) => {
    ...
    const { data, error } = await sb.from('asignaciones_serie')
        ...
        .single();
    if (error?.code === '23505')
        return c.json(err('ASIGNACION_DUPLICADA', 'Conflicto con otra asignación activa con los mismos criterios'), 422);
    if (error)
        return c.json(err('DB_ERROR', error.message), 500);
    if (!data)
        return c.json(err('NOT_FOUND', 'Asignación no encontrada'), 404);
...
configEmisionRoutes.post('/asignaciones/simular', async (c) => {
    ...
    const { data: asignaciones } = await sb.from('asignaciones_serie')
        ...
        .eq('activa', true)
        .order('prioridad');
    const match = (asignaciones ?? []).find((a) => (!a.tipo_tercero || a.tipo_tercero === body.tipo_tercero) &&
