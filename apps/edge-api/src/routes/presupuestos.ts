...
export const presupuestosRoutes = new Hono<{ Bindings: Env }>();
...
presupuestosRoutes.post('/', async (c) => {
  ...
  const body = await c.req.json<{ expediente_id: string; parte_id?: string }>();
...
presupuestosRoutes.post('/:id/recalcular', async (c) => {
  ...
  if (lineasError) return c.json({ data: null, error: { code: 'DB_ERROR', message: lineasError.message } }, 500);
