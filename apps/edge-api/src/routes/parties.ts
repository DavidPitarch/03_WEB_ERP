...
export const partiesRoutes = new Hono<{ Bindings: Env }>();
...
partiesRoutes.get('/', async (c) => {
  ...
  let query = supabase
    .from('parties')
    .select(
      `id, party_type, display_name, is_active,
       party_phones(number, is_primary),
       party_emails(email, is_primary),
       party_system_roles(role_code)`,
      { count: 'exact' }
    )
    .eq('is_active', true)
...
partiesRoutes.get('/:id', async (c) => {
  ...
  const [partyRes, phonesRes, emailsRes, addressesRes, idsRes, rolesRes] =
    await Promise.all([
...
partiesRoutes.post('/', async (c) => {
  ...
  const check = validate(body, {
    party_type: { required: true, isEnum: ['person', 'organization'] as const },
...
partiesRoutes.post('/:id/phones', async (c) => {
  ...
  const check = validate(body, {
    ...
    phone_type:  { isEnum: ['mobile','home','work','fax','other'] as const },
...
partiesRoutes.post('/:id/emails', async (c) => {
  ...
  const check = validate(body, {
    ...
    email_type: { isEnum: ['personal','work','other'] as const },
    label:      { maxLength: 100 },
  });
  if (!check.ok) return validationError(c, check.errors);
...
partiesRoutes.post('/:id/addresses', async (c) => {
  ...
  const check = validate(body, {
    ...
    address_type: {
      isEnum: ['home','work','billing','shipping','risk_location','postal','other'] as const,
    },
    label: { maxLength: 100 },
  });
  if (!check.ok) return validationError(c, check.errors);
...
partiesRoutes.post('/:id/identifiers', async (c) => {
  ...
  const check = validate(body, {
    id_type:  { required: true, isEnum: ['dni','nie','cif','passport','other'] as const },
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      id_value:   (body.id_value as string).toUpperCase().trim(),
      country:    body.country ?? 'ESP',
      is_primary: body.is_primary ?? false,
    })
    .select()
...
partiesRoutes.post('/merge', async (c) => {
  ...
  const body = await c.req.json<{ survivor_id: string; merged_id: string; reason?: string }>();
...
partiesRoutes.patch('/duplicate-candidates/:candidateId', async (c) => {
  ...
  const check = validate(body, {
    status: {
      required: true,
      isEnum: ['confirmed_duplicate', 'confirmed_different'] as const,
    },
    notes: { maxLength: 500 },
  });
  if (!check.ok) return validationError(c, check.errors);
