...
function normalizeEstadoCobro(value) {
    if (!value)
        return undefined;
    const normalized = value.toLowerCase();
    const aliases = {
...
function mapFacturaListadoRow(row) {
    return {
        ...row,
...
facturasRoutes.get('/caducadas', async (c) => {
    ...
    if (dias_min)
        query = query.gte('dias_vencida', Number(dias_min));
    const { data, error } = await query;
    if (error)
        return c.json(err('DB_ERROR', error.message), 500);
    return c.json({ data, error: null });
...
facturasRoutes.post('/emitir', async (c) => {
    ...
    const { data: serie } = await supabase
        ...
        .single();
    if (!serie)
        return c.json(err('SERIE_NO_ENCONTRADA', 'Serie de facturación no encontrada'), 422);
    if (!serie.activa)
        return c.json(err('SERIE_INACTIVA', 'La serie de facturación no está activa'), 422);
    ...
    if (serieErr)
        return c.json(err('DB_ERROR', serieErr.message), 500);
    ...
    const { data: factura, error: facturaErr } = await supabase
        ...
        .single();
    if (facturaErr)
        return c.json(err('DB_ERROR', facturaErr.message), 500);
    ...
    if (lineasPres && lineasPres.length > 0) {
        const lineasFactura = lineasPres.map((l) => ({
...
facturasRoutes.get('/', async (c) => {
    ...
    if (error)
        return c.json(err('DB_ERROR', error.message), 500);
    return c.json({
        data: (data ?? []).map((row) => mapFacturaListadoRow(row)),
        error: null,
...
facturasRoutes.post('/:id/enviar', async (c) => {
    ...
    const canal = body.canal
        ?? factura.companias?.config?.facturacion?.canal_envio
        ?? 'email';
    // 3-4. Set envio fields
    let envio_resultado = canal === 'email' ? 'ok' : 'pendiente_integracion';
    let envio_error = null;
    const now = new Date().toISOString();
    if (canal === 'email') {
        const companiaConfig = factura.companias?.config ?? {};
        ...
        const result = await sendFacturaEmail(supabase, c.env.RESEND_API_KEY, {
            id: factura.id,
            numero_factura: factura.numero_factura,
            total: factura.total,
            compania_email: companiaEmail,
            compania_nombre: factura.companias?.nombre ?? 'Cliente',
            empresa_nombre: factura.empresas_facturadoras?.nombre ?? 'ERP Siniestros',
            expediente_numero: factura.expedientes?.numero_expediente ?? factura.expediente_id,
        }, user.id);
...
facturasRoutes.post('/:id/registrar-cobro', async (c) => {
    ...
    if (factura.estado === 'anulada')
        return c.json(err('FACTURA_ANULADA', 'No se puede registrar cobro en factura anulada'), 422);
    if (!body.importe || body.importe <= 0)
        return c.json(err('IMPORTE_INVALIDO', 'El importe debe ser mayor que 0'), 422);
    // 1. Insert pago
    const { data: pago, error: pagoErr } = await supabase
        ...
        .single();
    if (pagoErr)
        return c.json(err('DB_ERROR', pagoErr.message), 500);
    // 2. Calculate total_cobrado
    const { data: pagos } = await supabase
        .from('pagos')
        .select('importe')
        .eq('factura_id', id);
    const total_cobrado = (pagos ?? []).reduce((sum, p) => sum + (p.importe ?? 0), 0);
...
facturasRoutes.post('/:id/reclamar', async (c) => {
    ...
    const body = await c.req.json();
    if (!body.contenido)
        return c.json(err('VALIDATION', 'contenido es requerido'), 422);
    const { data: factura } = await supabase
        .from('facturas')
    ...
    const { data: seguimiento, error: segErr } = await supabase
        ...
        .single();
    if (segErr)
        return c.json(err('DB_ERROR', segErr.message), 500);
