...
export const alertasRoutes = new Hono();
...
// ─── helpers ────────────────────────────────────────────────────────
function activeAlertFilter(query, userId) {
...
alertasRoutes.post('/generate', async (c) => {
    ...
    async function alertExists(tipo, ref) {
        let query = supabase
            .from('alertas')
        ...
        if (ref.expediente_id)
            query = query.eq('expediente_id', ref.expediente_id);
        if (ref.tarea_id)
            query = query.eq('tarea_id', ref.tarea_id);
