...
function dbError(c: any, err: unknown) {
  const msg = err instanceof Error ? err.message : String(err);
  const e = err as any;
  if (e?.status === 404) return c.json({ data: null, error: { code: 'NOT_FOUND',   message: e.message } }, 404);
  if (e?.status === 422) return c.json({ data: null, error: { code: e.code ?? 'VALIDATION', message: e.message, details: e.details } }, 422);
  return c.json({ data: null, error: { code: 'DB_ERROR', message: msg } }, 500);
}

const WRITE_ROLES = ['admin', 'supervisor'] as const;

function requireWriteRole(user: { roles?: string[] }): boolean {
...
calendarioRoutes.get('/festivos', async (c) => {
  ...
  if (comunidad_autonoma) q = q.eq('comunidad_autonoma', comunidad_autonoma);
...
calendarioRoutes.post('/festivos', async (c) => {
  ...
  const check = validate(body, {
    ...
    ambito:  { required: true, isEnum: ['nacional','autonomico','provincial','local','empresa'] as const },
  });
  ...
  if (error) {
    if (error.code === '23505') return c.json({ data: null, error: { code: 'DUPLICATE', message: 'Ya existe ese festivo para ese ámbito y fecha' } }, 409);
...
calendarioRoutes.get('/ausencias/:id', async (c) => {
  ...
  if (error) return c.json({ data: null, error: { code: 'NOT_FOUND', message: 'Ausencia no encontrada' } }, 404);
  return c.json({ data, error: null });
});
...
calendarioRoutes.post('/ausencias', async (c) => {
  ...
  const check = validate(body, {
    ...
    tipo:         { required: true, isEnum: ['vacacion','baja_medica','baja_laboral','asunto_personal','permiso_retribuido','bloqueo'] as const },
...
calendarioRoutes.put('/ausencias/:id', async (c) => {
  ...
  if (fetchErr || !current) return c.json({ data: null, error: { code: 'NOT_FOUND', message: 'Ausencia no encontrada' } }, 404);
...
calendarioRoutes.delete('/ausencias/:id', async (c) => {
  ...
  if (!current) return c.json({ data: null, error: { code: 'NOT_FOUND', message: 'Ausencia no encontrada' } }, 404);
...
calendarioRoutes.put('/ausencias/:id/rechazar', async (c) => {
  ...
  const supabase = c.get('adminSupabase');
  const body     = await c.req.json<{ motivo_rechazo?: string }>();
...
calendarioRoutes.post('/guardias', async (c) => {
  ...
  const check = validate(body, {
    operario_id:  { required: true, isUuid: true },
    tipo:         { required: true, isEnum: ['guardia','reten','turno_especial','disponibilidad_ampliada'] as const },
    fecha_inicio: { required: true },
    fecha_fin:    { required: true },
  });
  if (!check.ok) return validationError(c, check.errors);
...
calendarioRoutes.post('/excepciones', async (c) => {
  ...
  const check = validate(body, {
    tipo_excepcion: { required: true, isEnum: ['cita_en_festivo','cita_en_ausencia','fuera_horario','cita_en_bloqueo'] as const },
